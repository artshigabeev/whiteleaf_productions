...
function beau_instagram_image($username, $slice = 12) {
    ...
    // if (!$instagram = get_transient('instagram-media-'.sanitize_title_with_dashes($username))) {
        ...
        if (is_wp_error($remote))
            return new WP_Error('site_down', __('Unable to communicate with Instagram.', 'filmmaker'));

        if ( 200 != wp_remote_retrieve_response_code( $remote ) )
            return new WP_Error('invalid_response', __('Instagram did not return a 200.', 'filmmaker'));
        ...
        if (!$insta_array)
            return new WP_Error('bad_json', __('Instagram has returned invalid data.', 'filmmaker'));
