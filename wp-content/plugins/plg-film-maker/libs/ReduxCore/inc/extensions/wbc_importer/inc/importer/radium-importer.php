...
if ( !class_exists( 'Radium_Theme_Importer' ) ) {
  ...
  class Radium_Theme_Importer {
    ...
    public function __construct() {
      ...
      $this->master_slider = $this->demo_files_path . $this->master_sliders_file_name;
    ...
    public function process_imports() {
      ...
      if($_POST['current_process'] == 'posts_content') {
          if ( !empty( $this->content_demo ) && is_file( $this->content_demo ) ) {
            ...
            if(!empty($this->master_sliders_file_name)) {
                echo json_encode(array('is_done_setting_pages' => 1));
                die();
            } else {
            ...
            echo json_encode(array('is_done_setting_pages' => 1));
            die();
      } elseif($_POST['current_process'] == 'setup_masterslider') {
            $this->process_masterslider($this->master_slider);
            echo json_encode(array('completed' => 1));
            die();
      }
    }
     ...
     */
    public function process_masterslider($file_name){
        if(!class_exists('MSP_Importer_Extens')) {
           return;
        } 
        $masterSliderDemo = new MSP_Importer_Extens;
        $fileJsonSlider = file_get_contents($file_name);
    ...
    public function set_demo_menu() {
        
       @include $this->demo_files_path . 'setting-page.php';       
        $setting['page_setting'] = array(
            'front' => 'Home',
            'blog' => 'Blog'
        );
        $setting['menu_setting'] = array();
        if(function_exists('setting_page')) {            
            $setting = setting_page();          
        }
        //Setting page 
        // Use a static front page
        $front = get_page_by_title($setting['page_setting']['front']);
        update_option( 'page_on_front', $front->ID );
        update_option( 'show_on_front', 'page' );       
        // Set the blog page
        $blog   = get_page_by_title($setting['page_setting']['blog']);
        update_option( 'page_for_posts', $blog->ID );  
        // Setting menu
		$locations = array();
        foreach($setting['menu_setting'] as $menu_location => $menu_name){
